...
def create_job_post(
    ...
    return  service.create_job_post(current_user.id, job_data)
...
def get_job_posts(
    ...
    service = JobPostService(db)
    return  service.get_active_job_posts(
        page=page,
        per_page=per_page,
        job_type=job_type,
        location=location,
...
def get_my_job_posts(
    ...
    service = JobPostService(db)
    return  service.get_user_job_posts(current_user.id, page=page, per_page=per_page)
...
def get_job_post(
    ...
    service = JobPostService(db)
    job =  service.get_job_post(job_id)
    
    if not job:
        raise HTTPException(status_code=404, detail="Job posting not found")
    
    # Increment view count
    service.increment_job_views(job_id)
...
def update_job_post(
    ...
    service = JobPostService(db)
    job =  service.update_job_post(job_id, current_user.id, job_data)
...
def delete_job_post(
    ...
    service = JobPostService(db)
    success =  service.delete_job_post(job_id, current_user.id)
